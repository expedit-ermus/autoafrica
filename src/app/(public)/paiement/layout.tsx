import type { Metadata } from 'next';
import { SITE_URL } from '@/lib/structured-data';
import { BreadcrumbStructuredData } from '@/components/StructuredData';

export const metadata: Metadata = {
  title: 'Paiement Mobile Money — Pièces auto Abidjan',
  description:
    'Payez vos pièces auto en toute sécurité par Wave, Orange Money, MTN MoMo ou Moov Money à Abidjan. Reçu électronique inclus.',
  alternates: {
    canonical: `${SITE_URL}/paiement`,
  },
  openGraph: {
    title: 'Paiement Mobile Money sécurisé — AutoAfrique Abidjan',
    description:
      'Wave, Orange Money, MTN MoMo, Moov Money : payez vos pièces détachées auto à Abidjan en quelques minutes.',
    url: `${SITE_URL}/paiement`,
    type: 'website',
  },
};

export default function PaiementLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      <BreadcrumbStructuredData
        items={[
          { name: 'Accueil', url: SITE_URL },
          { name: 'Paiement', url: `${SITE_URL}/paiement` },
        ]}
      />
      {children}
    </>
  );
}
