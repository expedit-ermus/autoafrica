          ...
          <div className="fixed top-0 left-0 bottom-0 w-80 max-w-[85vw] bg-[var(--color-secondary)] z-50 md:hidden flex flex-col shadow-2xl overflow-y-auto">
            ...
            <div className="px-5 py-4 border-b border-white/10">
              <Link
                href={user ? '/dashboard' : '/auth/login'}
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent-warm)] rounded-xl text-white font-bold"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                {user ? L('Mon Dashboard', 'My Dashboard') : L('Se connecter', 'Log in')}
              </Link>
            </div>
