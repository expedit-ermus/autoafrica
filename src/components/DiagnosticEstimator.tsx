...
export default function DiagnosticEstimator() {
  ...
  return (
    <section className="py-12 md:py-18 bg-white border-y border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Titre de la section */}
        <div className="text-center max-w-3xl mx-auto mb-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-50 text-amber-800 text-xs font-black uppercase tracking-wider mb-3 border border-amber-200/80">
            <span>💡</span>
            ...
            <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-800 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-5 mb-6">
                ...
                <div className="text-right">
                  <span className="text-xs text-slate-400">{L('Durée moyenne', 'Avg duration')}</span>
              ...
              {/* Détail financier */}
              <div className="bg-slate-800/80 rounded-2xl p-5 space-y-3 mb-6 border border-slate-700/60">
                ...
                <div className="pt-3 border-t border-slate-700/80 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="text-[10px] text-slate-400 block uppercase tracking-wider">{L('Total Estimé', 'Total Est.')}</span>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {MECHANICS_DB.map((mec) => (
                    <div key={mec.id} className="p-3 rounded-2xl bg-slate-800/50 border border-slate-700/50 flex items-center justify-between text-xs">
                      <div>
                        <div className="font-bold text-white">{mec.name}</div>
                        <div className="text-[11px] text-slate-400">{mec.location} • <span className="text-amber-400 font-semibold">{mec.rating}</span></div>
                      </div>
                      <span className="text-[10px] px-2.5 py-1 bg-emerald-500/20 text-emerald-300 rounded-lg font-bold">
                        {mec.availability}
                      </span>
                    </div>
                  ))}
              ...
              {booked ? (
                ...
                <button
                  ...
                  className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-black text-sm sm:text-base transition-all shadow-xl shadow-orange-950/40 flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01] active:scale-[0.98]"
                >
                  <span>📲</span>
